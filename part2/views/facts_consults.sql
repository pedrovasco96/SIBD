...
(SELECT dim_animal.name, dim_animal.VAT_owner, dim_date.date_timestamp, count(distinct op.num) as 'distinct procedures', count(distinct pre.med_name) 'distinct medications'
	FROM
		dim_date, dim_animal, consult
		left outer join operation op
			on (op.name, op.VAT_owner, op.date_timestamp) = (consult.name, consult.VAT_owner, consult.date_timestamp)
